...
class ComputeImageSim(nn.Module):
    def __init__(self, N):
        super(ComputeImageSim, self).__init__()
        #self.avg_pool = nn.AvgPool2d((5, 5), stride=(5, 5))
        self.avg_pool = nn.AvgPool2d((N, N), stride=(5, 5))

    def image_similarity(self, images):
        batch_size = images.shape[0]
        images_similarity = torch.zeros(batch_size, batch_size)
        images = images.view(images.shape[0], -1).to('cuda')
        images_normalized = torch.nn.functional.normalize(images, dim=1).to('cuda')
        image_similarity_matrix = torch.matmul(images_normalized, images_normalized.T).to('cuda')
        #image_similarity = image_similarity.unsqueeze(0)
        #image_similarity_matrix = self.avg_pool(image_similarity).squeeze(0)
        image_similarity_matrix = (image_similarity_matrix + 1) / 2

        return image_similarity_matrix

class ComputeCider(nn.Module):
    def __init__(self, N):
        super(ComputeCider, self).__init__()
        self.avg_pool = nn.AvgPool2d((5, 5), stride=(5,5))
...
class ContrastiveLoss(nn.Module):
    ...
    def rank_matrix(self, sim_matrix):
        N = len(sim_matrix)
        sim_expand = sim_matrix.unsqueeze(-1).to('cuda') - sim_matrix[None, :, :].to('cuda')
        diag_mask = (1 - torch.eye(N)).unsqueeze(1).to('cuda')
        sim_rank = diag_mask * self.sigmoid(tensor = sim_expand, temp = 0.001).to('cuda')
        sim_rank_all = sim_rank.sum(0).to('cuda') +1
        return sim_rank_all

    # Compute MSC
    # -----------------For basic experiments--------------------------

    def main_semantic_consistency(self, scores, image_cider, image_similarity_matrix):
        score_rank = self.rank_matrix(scores)
        image_cider_rank = self.rank_matrix(image_cider)
        image_similarity_matrix_rank = self.rank_matrix(image_similarity_matrix)
        img_min_rank = torch.min(torch.min(image_cider_rank, score_rank), image_similarity_matrix_rank)
        img_max_rank = torch.max(torch.max(image_cider_rank, score_rank), image_similarity_matrix_rank)
        loss = (img_min_rank / img_max_rank).sum() / (len(scores)**2)
        return 10 * (1-loss)

    # ------------------For ablation study------------------------------

    # def texual_semantic_consistency(self, scores, caption_cider):
    #     score_rank = self.rank_matrix(scores)
    ...
    def forward(self, scores, image_cider, image_similarity_matrix, caption_cider):
        loss1 = self.triplet_loss(scores)
        loss2 = self.main_semantic_consistency(scores, image_cider, image_similarity_matrix)
        return loss1, loss2

    # ------------------For ablation study------------------------------

    # def forward(self, scores, image_cider, caption_cider, image_similarity_matrix):
    #      loss1 = self.triplet_loss(scores)
    #      loss2 = self.main_semantic_consistency(scores, image_cider, image_similarity_matrix) + self.texual_semantic_consistency(scores, caption_cider)
    #      return loss1, loss2
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        self.cider_com = ComputeCider(opt.batch_size)
        self.img_sim_com = ComputeImageSim(opt.batch_size)
    ...
    def forward_loss(self, sims, image_cider, caption_cider, image_similarity_matrix, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        triplet_loss, msc_loss = self.criterion(sims, image_cider, caption_cider, image_similarity_matrix)

        loss = triplet_loss + msc_loss

        self.logger.update('Loss', loss.item(), sims.size(0))
        self.logger.update('Tripliet_loss', triplet_loss.item(), sims.size(0))
        self.logger.update('Main_Semantic_Consistency_loss', msc_loss.item(), sims.size(0))
        return loss, triplet_loss, msc_loss
    ...
    def train_emb(self, images, captions, lengths, ids=None, tfidf_for_caption=None, tfidf_for_image=None, *args):
        ...
        #compute image-silimarity
        image_similarity_matrix = self.img_sim_com.image_similarity(images)

        # measure accuracy and record loss
        self.optimizer.zero_grad()
        loss, triplet_loss, sim_loss = self.forward_loss(sims, image_cider, caption_cider, image_similarity_matrix)
