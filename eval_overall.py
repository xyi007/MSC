...
def i2t(npts, sims, return_ranks=False):
    ...
    for index in range(npts):
        ...
        for i in range(5 * index, 5 * index + 5, 1):
            ...
            if tmp < rank:
                rank = tmp
        #image_inds = np.argsort(sims.T[5 * index])[::-1]
        #with open('/home/xy/project/MSC/inds.txt', 'a') as f:
        #    f.write(f"image_inds:{image_inds[0]}\n")
        #    f.write(f"text_inds:{inds[0:10]}\n")
        #    f.close()
        #print('image_inds:', image_inds[0])
        #print('text_inds:', inds[0:10])
        ranks[index] = rank
        top1[index] = inds[0]
...
def t2i(npts, sims, return_ranks=False):
    ...
    for index in range(npts):
        for i in range(5):
            ...
            #image_inds = np.argsort(sims.T[index+i])[::-1]
            #print('image_inds:', image_inds[0:10])
            #print('text_inds:', inds[0:10])
            ranks[5 * index + i] = np.where(inds == index)[0][0]
            top1[5 * index + i] = inds[0]

        text_inds = np.argsort(sims.T[index])[::-1]
        with open('/home/xy/project/MSC/inds-2.txt', 'a') as f:
            f.write(f"text_inds:{text_inds[0]}\n")
            f.write(f"image_inds:{inds[0:10]}\n")
            f.close()
        #print('text_inds:', text_inds[0])
        #print('image_inds:', inds[0:10])
